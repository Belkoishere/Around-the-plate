...
if ($stmt = $con->prepare('SELECT id, password FROM accounts WHERE username = ?')) {
    ...
    $stmt->execute();
    // Store the result so we can check if input account exists in database
    $stmt->store_result();
    ...
    if ($stmt->num_rows > 0) {
        ...
        //compare input password to hashed password from database
        ...
        if ($verify) {
            ...
            // Declare session variables that will be stored on the server
